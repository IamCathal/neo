...
func CreateCrawlingStatus(crawlingStatus dtos.SaveCrawlingStatsDTO) (bool, error) {
	crawlingStatusJSON, err := json.Marshal(crawlingStatus)
	if err != nil {
		configuration.Logger.Sugar().Infof("error marshaling crawlingStatus: %+v")
		return false, err
	}
	...
	if err != nil {
		configuration.Logger.Sugar().Infof("error creating POST /savecrawlingstats request: %+v", err)
		return false, err
	}
	req.Close = true
